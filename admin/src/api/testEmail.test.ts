...
test('test email uses the protected POST route and returns safe status', async () => {
  vi.stubGlobal('window', { FlowMailSMTPConfig: { restUrl: 'https://example.test/wp-json/flowmail-smtp/v1/', nonce: 'test-nonce' } });
  ...
  expect(String(url)).toBe('https://example.test/wp-json/flowmail-smtp/v1/test-email');
